...
const LS_KEY = "verusMatchHistoryByWeekV1";
const OPPONENTS_LS_KEY = "verusSavedOpponentsV1";

const openBtn = document.getElementById("verusMatchHistoryBtn");
const modal = document.getElementById("verusMatchHistoryModal");
const closeBtn = document.getElementById("verusMatchHistoryModalCloseBtn");
const statusEl = document.getElementById("verusMatchHistoryStatus");
const weekInput = document.getElementById("verusMatchHistoryWeekStart");
const saveBtn = document.getElementById("verusMatchHistorySaveBtn");
const weekListEl = document.getElementById("verusMatchHistoryWeekList");
const searchOppEl = document.getElementById("verusMatchHistorySearchOpp");
const selectVisibleBtn = document.getElementById("verusMatchHistorySelectVisibleBtn");
const clearSelBtn = document.getElementById("verusMatchHistoryClearSelBtn");
const compareBtn = document.getElementById("verusMatchHistoryCompareBtn");
const addCompareBtn = document.getElementById("verusMatchHistoryAddCompareBtn");
const deleteSavedBtn = document.getElementById("verusMatchHistoryDeleteSavedBtn");
const comparePanel = document.getElementById("verusMatchHistoryComparePanel");
const compareTitle = document.getElementById("verusMatchHistoryCompareTitle");
const compareScroll = document.getElementById("verusMatchHistoryCompareScroll");
const compareHideBtn = document.getElementById("verusMatchHistoryCompareHideBtn");
const ourLineup = document.getElementById("verusMHOurLineup");
const ourPlan = document.getElementById("verusMHOurPlan");
const theirLineup = document.getElementById("verusMHTheirLineup");
const theirPlan = document.getElementById("verusMHTheirPlan");
const oppName = document.getElementById("verusMHOpponentName");
const oppServer = document.getElementById("verusMHOpponentServer");
const oppPower = document.getElementById("verusMHOpponentAlliancePower");
const matchResultWinCb = document.getElementById("verusMHResultWin");
const matchResultLossCb = document.getElementById("verusMHResultLoss");
const allianceRecordEl = document.getElementById("verusMHAllianceRecord");
const saveOpponentBtn = document.getElementById("verusMHSaveOpponentBtn");
const importOpponentSel = document.getElementById("verusMHImportOpponent");
...
function readTop5FromDom() {
  ...
  const out = [];
  for (let i = 0; i < 5; i++) {
    const n = root.querySelector('.ds-mh-op-name[data-idx="' + i + '"]');
    const p = root.querySelector('.ds-mh-op-power[data-idx="' + i + '"]');
    out.push({
...
/** Aggregate wins/losses from all saved match weeks (one result per week if set). */
...
function renderWeekList() {
  ...
  if (keys.length === 0) {
    ...
    opt0.textContent = searchQueryNormalized()
      ...
      : "No saved matches yet. Save the current match date to add one.";
  } else {
...
function renderComparePanel() {
  ...
  if (compareTitle) {
    compareTitle.textContent =
      keys.length === 1 ? "1 week selected" : "Comparing " + keys.length + " weeks";
  }
  comparePanel.removeAttribute("hidden");
  comparePanel.setAttribute("aria-hidden", "false");
  if (statusEl) statusEl.textContent = "Showing comparison for " + keys.length + " week(s).";
}
...
function openModal() {
  ...
  mon = normalizeMatchDateKey(mon) || normalizeMatchDateKey(formatYmd(new Date()));
...
if (!firebaseConfigOk(cfg)) {
  ...
  try {
    ...
    console.warn("[Last War Tools] verus-tracker-match-history:", e);
    setOpenButtonVisible(false);
  }
}
